...
public class Indexer
{
    ...
    {   
        if (writer == null)
        {
            writer = new IndexWriter(FSDirectory.open(
                new File(indexDir)), new IndexWriterConfig(Version.LUCENE_900, new StandardAnalyzer(Version.LUCENE_900))
                );
        }        
    } 
    //add items into index
    ...
    { // deleting the item, if already exists 
        ...
        doc.add(new Field(IndexItem.ARTIST, indexItem.getArtist(), Field.Store.YES, Field.Index.ANALYZED));
        doc.add(new Field(IndexItem.TITLE, indexItem.getTitle(), Field.Store.YES, Field.Index.ANALYZED));
        doc.add(new Field(IndexItem.TEXT, indexItem.getText(), Field.Store.YES, Field.Index.ANALYZED));
